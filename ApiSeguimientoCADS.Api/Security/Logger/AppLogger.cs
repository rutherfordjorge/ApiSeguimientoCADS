...
namespace ApiSeguimientoCADS.Api.Security.Logger
{
    ...
    /// <remarks>
    /// Los campos estáticos en este tipo genérico son intencionales y están diseñados para
    /// optimizar el rendimiento mediante el uso de LoggerMessage delegates. Cada instanciación
    /// del tipo genérico (AppLogger&lt;T&gt;) mantiene su propio conjunto de campos estáticos,
    /// lo cual es el comportamiento deseado para cachear los delegados de logging por tipo.
    /// Esta es una práctica recomendada por Microsoft para logging de alto rendimiento.
    /// </remarks>
    public class AppLogger<T> : IAppLogger<T>
    {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Minor Code Smell", "S6672:Generic logger injection should match enclosing type", Justification = "AppLogger<T> is a generic wrapper where T represents the actual logging context type, not the wrapper itself.")]
        ...
        /// <returns>Instancia del logger.</returns>
        public IAppLogger<T> LogError(Exception ex, string message)
        {
            ArgumentNullException.ThrowIfNull(ex);
            ArgumentNullException.ThrowIfNull(message);

            // Log el mensaje personalizado con la excepción
            _logErrorMessage(this._logger, $"{_redAsciiColor}{message}: {ex.Message}", ex);
        ...
        {
            var id = Guid.NewGuid();
            var correlationId = NLog.ScopeContext.TryGetProperty("X-Request-ID", out var value) && value != null
                ? value.ToString()
                : "N/A";
            this._key = $"{_genericName}|{correlationId}|{id}";
        ...
        private IAppLogger<T> Log(LogLevel level, params object?[] messages)
        {
            ...
            // Log el key solo una vez al principio
            if (processedMessages.Length > 0)
            {
                _logKeyMessage(this._logger, $"{_greenAsciiColor}At {this._key}", null);
            }

            // Log todos los mensajes sin repetir el key
