...
namespace ApiSeguimientoCADS.Api.Handlers
{
    ...
    public class HttpHandler : IHttpHandler
    {
            ...
            where TResponse : class
        {
            ...
            {
                this._logger.LogError(ex, "Se produjo un error en los argumentos de la solicitud.");
                throw;
            }
            catch (InvalidOperationException ex)
            {
        ...
        where TResponse : class
        {
            ...
            try
            {
                ...
                var tasks = urlList.Select(url => this._httpService.GetAsync<TResponse>(url, bearerToken));
