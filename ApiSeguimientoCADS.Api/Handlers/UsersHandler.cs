...
namespace ApiSeguimientoCADS.Api.Handlers
{
    ...
    {
        private const string _httpPrefix = "http://";
        private const string _httpsPrefix = "https://";
        ...
        private static async Task<string> RedirectToOrigin(string urlOrigen)
        {
            ...
            try
            {
                ...
                if (!Uri.TryCreate(_httpPrefix + urlOrigen, UriKind.Absolute, out Uri? testUri))
                {
                    // Si no es válida, devolver con https por defecto
                    return _httpsPrefix + urlOrigen;
                }

                bool result = await Helper.UrlExistsAsync(testUri).ConfigureAwait(false);
                string prefix = result ? _httpPrefix : _httpsPrefix;
