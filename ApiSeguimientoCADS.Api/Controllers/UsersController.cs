...
namespace ApiSeguimientoCADS.Api.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        public async Task<IActionResult> Validate([FromForm] ValidateAccessRequest request)
        {
            ...
            {
                this._logger.LogError(ex, "Se detectó una excepción de memoria insuficiente durante el procesamiento de la solicitud.");
                this._logger.EndProcess(processId, stopwatch, new { Success = false, Error = "OutOfMemoryException" });
                return this.StatusCode(StatusCodes.Status500InternalServerError, new { Message = "El servidor no dispone de suficiente memoria para completar la operación." });
            }
            catch (StackOverflowException ex)
            {
                this._logger.LogError(ex, "Se detectó un desbordamiento de pila durante el procesamiento de la solicitud.");
                this._logger.EndProcess(processId, stopwatch, new { Success = false, Error = "StackOverflowException" });
                return this.StatusCode(StatusCodes.Status500InternalServerError, new { Message = "Se produjo un error interno en el servidor (desbordamiento de pila)." });
            }
