...
namespace ApiSeguimientoCADS.Api.Controllers
{
    ...
    public class FakeController : ControllerBase
    {
        ...
        /// <param name="httpClientService">httpClientService de la aplicación.</param>
        public FakeController(IAppLogger<FakeController> logger, IUsersHandler handler, IOptions<EPSiniestroPorAseguradoSettings> settings, IHttpClientService httpClientService)
        {
        ...
        public async Task<string> LoginFake(string rut, string pass)
        {
            try
            {
                ...
                {
                    { this._settings.TokenCABName, this._settings.TokenCABValue },
                    { this._settings.CookieName, this._settings.CookieValue },
                };

                var responseBody = await this._httpClientService.PostAsync(new Uri(this._settings.ValidarAccesoMultipleUrl), jsonBody, headers).ConfigureAwait(false);

                // Parsear respuesta
                using var doc = JsonDocument.Parse(responseBody);

                string? sessionId = doc.RootElement.TryGetProperty("SessionId", out var sessionProp)
                ...
                return urlFinal.Replace(".desa.", ".qa.", StringComparison.OrdinalIgnoreCase);
            }
            ...
            {
                this._logger.LogError(invEx);
                return $"Error de operación no válida: {invEx.Message}";
            }
        }
