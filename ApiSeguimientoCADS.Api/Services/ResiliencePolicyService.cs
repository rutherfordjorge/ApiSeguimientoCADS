        ...
        private ResiliencePipeline CreateCircuitBreakerPipeline()
        {
            var circuitBreaker = new ResiliencePipelineBuilder()
                .AddCircuitBreaker(new CircuitBreakerStrategyOptions
                {
                    ...
                    OnClosed = args =>
                    {
                        this._logger.Info($"[Circuit Breaker] Circuito CERRADO - Servicio recuperado: {args}");
                        return ValueTask.CompletedTask;
                    },
                    OnHalfOpened = args =>
                    {
                        this._logger.Info($"[Circuit Breaker] Circuito SEMI-ABIERTO - Probando recuperación: {args}");
                        return ValueTask.CompletedTask;
                    },
                    ShouldHandle = new PredicateBuilder()
                        .Handle<HttpRequestException>()
