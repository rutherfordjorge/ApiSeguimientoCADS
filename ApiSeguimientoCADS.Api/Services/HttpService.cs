    ...
    public class HttpService : IHttpService
    {
            ...
            where TResponse : class
        {
            ...
            try
            {
                ...
                this._logger.EndProcess(processId, stopwatch, new { Success = result.IsSuccess, StatusCode = result.StatusCode });
            ...
            where TResponse : class
        {
            ...
            try
            {
                ...
                var result = await this._resiliencePolicyService.CircuitBreakerPipeline.ExecuteAsync(
                    async cancellationToken => await this._httpClientService.SendWithoutRetryAsync<TResponse>(apiRequest).ConfigureAwait(false),
            ...
            {
                return this.HandleTaskCanceledException(ex, response, processId, stopwatch, url, "GET with Circuit Breaker");
            }
            catch (HttpRequestException ex)
            {
                return this.HandleHttpRequestException(ex, response, processId, stopwatch, url, "GET with Circuit Breaker");
            }
        }
            ...
            where TResponse : class
        {
            ...
            {
                return this.HandleTaskCanceledException(ex, response, processId, stopwatch, url, "POST with Circuit Breaker");
            }
            catch (HttpRequestException ex)
            {
                return this.HandleHttpRequestException(ex, response, processId, stopwatch, url, "POST with Circuit Breaker");
            }
        }
            ...
            where TResponse : class
        {
            ...
            try
            {
                ...
                if (result.IsSuccess)
                {
                    ...
                    this._logger.EndProcess(processId, stopwatch, new { Success = true, StatusCode = result.StatusCode });
                    return result.Data;
                }

                this._logger.Info($"{methodName} request failed. Status: {result.StatusCode}");
                this._logger.EndProcess(processId, stopwatch, new { Success = false, StatusCode = result.StatusCode });
                return null;
            }
            catch (OutOfMemoryException)
            {
        ...
        private HttpApiResponse<TResponse> HandleUriFormatException<TResponse>(
            ...
            where TResponse : class
        {
            this._logger.LogError(ex, $"Error al construir la URL: {url}");
            this._logger.EndProcess(processId, stopwatch, new { Success = false, Error = "UriFormatException" });
